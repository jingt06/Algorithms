...
class Dynamic_Array{
	public:
		...
		int Binary_Search(int k,int front, int end){
			int mid = (end + front) / 2;
			while(end >= front){
				if(arr[mid]->key == k){
		...
		//Gallop Search, used on data streams, huge file, etc
		int Search(int k){
			int i = 0;
			while(i < size and k > arr[i]->key){
				i = 2*i + 1;
			}
			return Binary_Search(k, i/2, min(i, size - 1));
		}
